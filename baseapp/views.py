...
def stock(request, ticker):
    ...
    try:
        stock_data = get_historical_stock_data(ticker.upper()+".NS")
...
def searched(request):
    if request.method == 'GET':
        searched = request.GET.get('search').lower()
        return redirect('stock', ticker=searched)  # Redirect to the stock view
